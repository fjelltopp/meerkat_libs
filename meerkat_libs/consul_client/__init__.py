...
def send_dhis2_events(uuid=None, raw_row=None, form_id=None, auth_token=None):
    ...
    if len(events_buffer) > 500:
        ...
        __send_events_from_buffer(auth_token)
...
def __send_events_from_buffer(auth_token):
    global events_buffer
    json_payload = json.dumps(
        {"Messages": events_buffer}
    )
    requests.post(CONSUL_URL + "/dhis2/export/events", headers=_auth_headers(auth_token), json=json_payload)
    events_buffer = []
